...
class TestOvsNeutronAgent(object):
    ...
    def test_process_ports_events_returns_current_for_unchanged_ports(self):
        with mock.patch.object(self.agent, 'check_changed_vlans',
            ...
            devices_not_ready_yet = {
                'devices': set(), 'ancillary_devices': set()}
            actual = self.agent.process_ports_events(
                events, failed_devices,
                failed_ancillary_devices, devices_not_ready_yet)
    ...
    def test_process_port_events_with_updated_ports(self):
        ...
        with mock.patch.object(self.agent.int_br, 'portid_from_external_ids',
            ...
            actual = self.agent.process_ports_events(
                events, failed_devices,
                failed_ancillary_devices, devices_not_ready_yet, updated_ports)
            self.assertEqual((expected_ports, expected_ancillary), actual)
    ...
    def test_process_port_events_port_not_ready_yet(self):
        events = {'added': [{'name': 'port4', 'ofport': [],
                  'external_ids': {'attached-mac': 'test-mac'}}],
                  'removed': []}
        expected_ports = dict(added=set(['not_ready']), removed=set())
        expected_ancillary = dict(added=set(), removed=set())
        with mock.patch.object(self.agent.int_br, 'portid_from_external_ids',
                               side_effect=[4]), \
            mock.patch.object(self.agent, 'check_changed_vlans',
            ...
            failed_ancillary_devices = {
                'added': set(), 'removed': set()}
            devices_not_ready_yet = {
                'devices': set(['not_ready']), 'ancillary_devices': set()}
            devices_not_ready_yet_expected = {
                'devices': set([4]), 'ancillary_devices': set()}
            actual = self.agent.process_ports_events(
                events, failed_devices,
                failed_ancillary_devices, devices_not_ready_yet)
            self.assertEqual((expected_ports, expected_ancillary), actual)
            self.assertEqual(devices_not_ready_yet_expected,
                             devices_not_ready_yet)
    ...
    def test_update_ports_returns_changed_vlan(self):
        ...
        port_tags_dict = {1: []}
        expected_ports = dict(added=set([2]), removed=set(), updated=set([1]))
        with mock.patch.object(self.agent.int_br, 'portid_from_external_ids',
                              side_effect=[2]), \
            ...
            actual = self.agent.process_ports_events(
                events, failed_devices,
                failed_ancillary_devices, devices_not_ready_yet)
            self.assertEqual(expected_ports, actual)
    ...
    def test_bind_devices(self):
        ...
        with mock.patch.object(
            ...
            failed_devices = {'added': set(), 'removed': set()}
            self.agent._bind_devices(port_details, failed_devices)
    ...
    def _test_arp_spoofing(self, enable_prevent_arp_spoofing):
        ...
        with mock.patch.object(
            ...
            int_br.get_ports_attributes.return_value = ovs_db_list
            failed_devices = {'added': set(), 'removed': set()}
            self.agent._bind_devices(need_binding_ports, failed_devices)
    ...
    def _mock_treat_devices_added_updated(self, details, port, func_name):
        ...
        with mock.patch.object(self.agent.plugin_rpc,
            ...
            skip_devs, need_bound_devices, insecure_ports = (
                self.agent.treat_devices_added_or_updated([], failed_devices,
                    False))
        ...
        with mock.patch.object(self.agent.plugin_rpc,
                               ...
                               return_value={'devices_up': [],
                                             ...
                                             'failed_devices_down': [
                                                 dev_mock]}):
            failed_devices = {'added': set(), 'removed': set()}
            self.agent.treat_devices_removed([{}], failed_devices)
    ...
    def test_bind_port_with_missing_network(self):
        ...
        self.agent._bind_devices([{'network_id': 'non-existent',
                                   'vif_port': vif_port}], failed_devices)
        ...
        with mock.patch.object(self.agent.sg_agent,
            ...
            if devices_added_updated:
                device_added_updated.assert_called_once_with(
                    devices_added_updated, failed_devices, False)
            if port_info.get('removed', set()):
                device_removed.assert_called_once_with(
    ...
    def test_process_network_ports_with_insecure_ports(self):
        ...
        with mock.patch.object(self.agent.sg_agent,
            ...
            self.assertEqual((
                ...
                self.agent.process_network_ports(port_info, False))
            device_added_updated.assert_called_once_with(
                set(['eth1', 'tap1']), failed_dev, False)
            setup_port_filters.assert_called_once_with(
    ...
    def _test_ovs_status(self, *args):
        ...
        with mock.patch.object(async_process.AsyncProcess, "_spawn"),\
            ...
            devices_not_ready = {'devices': set(),
                                 'ancillary_devices': set()}
    ...
    def test_update_stale_ofport_rules_clears_old(self):
        ...
        failed_devices = {'added': set(), 'removed': set()}
        self.agent.update_stale_ofport_rules(failed_devices)
...
class TestOvsDvrNeutronAgent(object):
    ...
    def _test_treat_devices_removed_for_dvr_interface(
        ...
        with mock.patch.object(self.agent, 'reclaim_local_vlan'),\
            ...
            self.agent.treat_devices_removed(
                [self._port.vif_id], failed_devices)
            lvid = self.agent.local_vlan_map[self._net_uuid].vlan
            if ip_version == 4:
                expected = [
    ...
    def _test_treat_devices_removed_for_dvr(self, device_owner, ip_version=4):
        ...
        with mock.patch.object(self.agent, 'reclaim_local_vlan'),\
            ...
            failed_devices = {'added': set(), 'removed': set()}
            self.agent.treat_devices_removed(
                [self._compute_port.vif_id], failed_devices)
            int_br.assert_has_calls([
