...
class OVSNeutronAgent(sg_rpc.SecurityGroupAgentRpcCallbackMixin,
    ...
    def process_ports_events(self, events, registered_ports, ancillary_ports,
                             old_devices_not_ready, updated_ports=None):
        port_info = {}
        port_info['added'] = set()
                ...
                if iface_id:
                    if device['ofport'] == ovs_lib.UNASSIGNED_OFPORT:
                        LOG.debug("Port %s not ready yet on the bridge",
                                  iface_id)
                        devices_not_ready_yet.add(device['name'])
                        return
        ...
        if old_devices_not_ready:
            old_devices_not_ready_attrs = self.int_br.get_ports_attributes(
                'Interface', columns=['name', 'external_ids', 'ofport'],
                ports=old_devices_not_ready, if_exists=True)
            events['added'].extend(old_devices_not_ready_attrs)
        ...
        return port_info, ancillary_port_info, devices_not_ready_yet
    ...
    def process_port_info(self, start, polling_manager, sync, ovs_restarted,
                       ...
                       consecutive_resyncs, devices_not_ready_yet):
            ...
            if self._agent_has_updates(polling_manager) or sync:
                try:
                    ...
                    (port_info, ancillary_port_info, consecutive_resyncs,
                     devices_not_ready_yet) = (self.process_port_info(
