...
class OVSNeutronAgent(sg_rpc.SecurityGroupAgentRpcCallbackMixin,
    ...
    def treat_devices_removed(self, devices, failed_devices):
        ...
        devices_down = self.plugin_rpc.update_device_list(self.context,
                                                          ...
                                                          self.agent_id,
                                                          self.conf.host)
        failed_dev_down = devices_down.get('failed_devices_down', [])
        if failed_dev_down:
            for dev in failed_dev_down:
                failed_devices['removed'].add(dev)
            LOG.debug("Port removal failed for %(devices)s ", failed_dev_down)
    ...
    def process_ancillary_network_ports(self, port_info):
        ...
        if 'added' in port_info and port_info['added']:
            start = time.time()
            failed_devices = self.treat_ancillary_devices_added(
                port_info['added'])
        ...
        if 'removed' in port_info and port_info['removed']:
            start = time.time()
            failed_devices = self.treat_ancillary_devices_removed(
                port_info['removed'])
            if failed_devices:
                for dev in failed_devices:
                    failed_devices['removed'].add(dev)
