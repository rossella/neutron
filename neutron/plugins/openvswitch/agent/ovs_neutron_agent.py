...
class OVSNeutronAgent(sg_rpc.SecurityGroupAgentRpcCallbackMixin,
    ...
    def treat_devices_added_or_updated(self, devices, ovs_restarted):
        ...
        failed_devices = []
        if devices_up:
            devices_set_up = self.plugin_rpc.update_device_list_up(
                self.context, devices_up, self.agent_id, cfg.CONF.host)
            failed_devices.extend(devices_set_up.get('failed_devices'))
        if devices_down:
            devices_set_down = self.plugin_rpc.update_device_list_down(
                self.context, devices_down, self.agent_id, cfg.CONF.host)
            failed_devices.extend(devices_set_down.get('failed_devices'))
        if failed_devices:
        ...
        LOG.info(_LI("Configuration for devices %s completed."), devices)
    ...
    def treat_ancillary_devices_added(self, devices):
        ...
        # update plugin about port status
        devices_set_up = (
            self.plugin_rpc.update_device_list_up(self.context,
                                                  devices_added,
                                                  self.agent_id,
                                                  cfg.CONF.host))
        if devices_set_up.get('failed_devices'):
            #TODO(rossella_s) handle better the resync in next patches,
            # this is just to preserve the current behavior
            raise DeviceListRetrievalError()
    ...
    def treat_devices_removed(self, devices):
        ...
        LOG.info(_LI("Attachments %s removed"), devices)
        devices_down = self.plugin_rpc.update_device_list_down(
            self.context,
            devices,
            self.agent_id,
            cfg.CONF.host)
        failed_devices = devices_down.get('failed_devices')
        if failed_devices:
            LOG.debug("port_removed failed for %(devices)s: %(e)s",
    ...
    def treat_ancillary_devices_removed(self, devices):
        resync = False
        LOG.info(_LI("Attachments %s removed"), devices)
        details = self.plugin_rpc.update_device_list_down(self.context,
                                                          devices,
                                                          self.agent_id,
                                                          cfg.CONF.host)
        ...
        for detail in devices:
            if details['exists']:
                LOG.info(_LI("Port %s updated."), details['device'])
                # Nothing to do regarding local networking
            else:
                LOG.debug("Device %s not defined on plugin", details['device'])
        return resync
